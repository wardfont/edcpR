...
data(iris_s5, package = "edcpR")
head(iris_s5)
iris_s5 <- iris_s5[, c(1,5)]
colnames(iris_s5) <- c("sepal_length", "Species")
```
...
kruskal.test(sepal_length ~ Species, data = iris_s5)
```
...
dunnTest(sepal_length ~ Species, data = iris_s5, method= "bh")
```
...
```{r, echo=F, eval=T}
m<-mean(iris_s5$sepal_length)
std<-sqrt(var(iris_s5$sepal_length))

hist(iris_s5$sepal_length , prob=TRUE, xlab="Sepal lenghth",main="Histogram of sepal length")
curve(dnorm(x, mean=m, sd=std),col="darkblue", lwd=2, add=TRUE, yaxt="n")
```
...
```{r, echo=T, eval=T}
qqnorm(iris_s5$sepal_length, main="Normal Q-Q of sepal length")
qqline(iris_s5$sepal_length, col = "red", lwd = 2)
```
...
iris_s5$log_sl <- log(iris_s5$sepal_length)
iris_s5$sqrt_sl <- sqrt(iris_s5$sepal_length)
```

## Question 2 Solution (8)

```{r, echo=F, eval=T}
m<-mean(iris_s5$log_sl)
std<-sqrt(var(iris_s5$log_sl))

hist(iris_s5$log_sl , prob=TRUE, xlab="Log(Sepal lenghth)",main="Logaritmic transformation")
curve(dnorm(x, mean=m, sd=std),col="darkblue", lwd=2, add=TRUE, yaxt="n")
...
```{r, echo=T, eval=T}
qqnorm(iris_s5$log_sl, main="Logaritmic transformation")
qqline(iris_s5$log_sl, col = "red", lwd = 2)
```

## Question 2 Solution (10)

```{r, echo=F, eval=T}
m<-mean(iris_s5$sqrt_sl)
std<-sqrt(var(iris_s5$sqrt_sl))

hist(iris_s5$sqrt_sl , prob=TRUE, xlab="sqrt(Sepal lenghth)",main="Square Root Transformation")
curve(dnorm(x, mean=m, sd=std),col="darkblue", lwd=2, add=TRUE, yaxt="n")
```
...
```{r, echo=T, eval=T}
qqnorm(iris_s5$sqrt_sl, main="Square Root Transformation")
qqline(iris_s5$sqrt_sl, col = "red", lwd = 2)
```
...
# Statistical test for normality (Shapiro-Wilk)
# Not recommended, because it is too strict
shapiro.test(iris_s5$sepal_length)
```

```{r, echo=T, eval=F}
shapiro.test(iris_s5$log_sl)
shapiro.test(iris_s5$sqrt_sl)
```
...
leveneTest(sepal_length ~ Species, data = iris_s5)
```
...
# Compute the ANOVA
res.aov <- aov(sepal_length ~ Species, data = iris_s5)
summary(res.aov)
```
